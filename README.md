...
#let convert_math(cont) = {
  ...
  let temp = cont.text.replace("\t", "\\t")
  // caution! typst has problem dealing with string, thats why we cannot mannually replace \n, it may appears inside math equation. Also makes regex more complicated https://github.com/typst/typst/issues/5460
  // temp = temp.replace("\n", "\\n")
  // temp = temp.replace("\r", "\\r")
  "$"
  // decide whether it is inline math
  if cont.text.starts-with("\\(") {
    mitex-convert(mode: str, temp)
  }
  else {mitex-convert(temp)} // cannot use mitex-convert(mode: str, temp), it causes failure and I don`t know why... 
  "$"
}
